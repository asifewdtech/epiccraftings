...
function cost_calcultor(nText1, nText2, nText3) {
    ...
    if ($('input[value="centimeter"]').prop("checked")) {
        let widthHeightCm = (Math.round(finalWidth) * 2.54).toFixed(0) + "x" + (Math.round(finalHeight) * 2.54).toFixed(0);
            let text_Arr = widthHeightCm.split('x');
            textWidth.innerText = text_Arr[0]+" cm";
            textHeight.innerHTML = text_Arr[1]+" cm";
        
     }
    if ($('input[value="inches"]').prop("checked")) {
...
function widthConvert(e) {
    if ($(e).val()=="centimeter") {
        var width= $('#textWidth').text();
        width.replace('inches','')
        var height= $('#textHeight').text();
        height.replace('inches','')

        let widthHeightCm = (Math.round(parseInt(width)) * 2.54).toFixed(0) + "x" + (Math.round(parseInt(height)) * 2.54).toFixed(0);
            let text_Arr = widthHeightCm.split('x');
            textWidth.innerText = text_Arr[0]+" cm";
            textHeight.innerHTML = text_Arr[1]+" cm";
    ...
    if ($(e).val()=="inches") {
        ...
        let widthHeightInches = (Math.round(parseInt(width)) / 2.54).toFixed(0) + "x" + (Math.round(parseInt(height)) / 2.54).toFixed(0);
...
setInterval(function() {
    var i = 1;
    $.each($(".font-div"), function() {
        $(this).html("<span style='font-family:Arial !important;' class='top-tabs pl-1 pr-3'>" + i + "</span><span>" + $($("#input-font-id").val()).val().substring(0, 10) + "</span>");
        i++;
    });
